...
class DataBuffer():
    ...
    def read_atrocities(self, day, data):
        ...
        for country in range(COUNTRIES):
            for pid in range(len(PERIODS)):
                ...
                self.rolling_add(self.curr_country_atroc[COUNTRIES], pid, self.all_cnt_atro[country], day, period)
                self.rolling_add(self.trailing30_country_atroc[country], pid, self.all_cnt_atro[country],day, period, trailing=30)
                self.rolling_add(self.trailing30_country_atroc[COUNTRIES], pid, self.all_cnt_atro[country],day, period, trailing=30)
    ...
    def create_features(line):
        #We already do this in	read_news_data
        #line = line.strip().split()
    ...
    def read_geography(self, inputData):
          ...
          while (i < (len(inputData))):
              countryID = int(inputData[i].split(" ")[0])
              i += 1
              regionID = int(inputData[i].split(" ")[0])
              i += 1
              #print(countryID, regionID)				  
              self.map_insert(self.country_to_regions, countryID, regionID)
              self.map_insert(self.region_to_country, regionID, countryID)
              outer = False
              while (i < len(inputData) and (inputData[i].rstrip('\n') == 'outer' 
                  or inputData[i].rstrip('\n') == 'inner')):
                  if (outer == False and inputData[i].rstrip('\n') == 'outer'):
                      i += 1
                      co_ordinates = inputData[i].split(" ")[0]
                      co_ordinates = co_ordinates.split(",")
                      #mapInsert(region_long, regionID, co_ordinates[0])
                      #mapInsert(region_latt, regionID, co_ordinates[1])
                      self.region_geo[regionID] = co_ordinates
                      outer = True
                      i += 1
                  else:
                      i += 2	
...
class Receiver():
    ...
    def receive_data(self, source_type, day, data):
        ...
        if source_type == 0:  # this is atrocity data
            ...
            self.library = Library(REGIONS, COUNTRIES, PERIODS)
            self.buf = DataBuffer(self.library)  # this will be called before the other two
