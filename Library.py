...
class Library():
    ...
    def create_data(self, buffer, day):
        for reg in range(self.region_count):
            ...
            #Since KEYs are string and its list of list
            country_id = buffer.region_to_country[reg][0]  # country to which this region belongs
            ...
            self.train_data[self.position].extend([buffer.trailing30_country_atroc[self.world_index][5]])  # 35 day trail
            if reg in buffer.reg_atroc_dates:
                self.train_data[self.position].extend([buffer.last_atrocity(buffer.reg_atroc_dates[reg], day, tr=30)])
            else:
                self.train_data[self.position].extend([buffer.last_atrocity(None, day, tr=30)])	
            if country_id in buffer.cnt_atroc_dates:				
                self.train_data[self.position].extend([buffer.last_atrocity(buffer.cnt_atroc_dates[country_id], day, tr=30)])
            else:
                self.train_data[self.position].extend([buffer.last_atrocity(None, day, tr=30)])					
            ...
            self.predict_data[reg].extend([buffer.curr_country_atroc[self.world_index][5]])  # 35 day trail
            if reg in buffer.reg_atroc_dates:			
                self.predict_data[reg].extend([buffer.last_atrocity(buffer.reg_atroc_dates[reg], day)])
            else:
                self.predict_data[reg].extend([buffer.last_atrocity(None, day)])
            if country_id in buffer.cnt_atroc_dates:
                self.predict_data[reg].extend([buffer.last_atrocity(buffer.cnt_atroc_dates[country_id], day)])
            else:
                self.predict_data[reg].extend([buffer.last_atrocity(None, day)])
