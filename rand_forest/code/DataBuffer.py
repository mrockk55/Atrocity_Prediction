...
class DataBuffer():
    ...
    def read_atrocities(self, day, data):
        ...
        for country in range(COUNTRIES):
            for pid in range(len(PERIODS)):
                ...
                self.rolling_add(self.trailing30_country_atroc[COUNTRIES], pid, self.all_cnt_atro[country],
                                 day, period, trailing=30)
    ...
    def read_geography(self, inputData):
          ...
          while (i < (len(inputData))):
              countryID = inputData[i].split(" ")[0]
              i += 1
              regionID = inputData[i].split(" ")[0]
              i += 1
              #print(countryID, regionID)				  
              self.map_insert(self.country_to_regions, int(countryID), int(regionID))
              self.map_insert(self.region_to_country, int(regionID), int(countryID))
              outer = False
